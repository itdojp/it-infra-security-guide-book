...
**多要素認証（MFA）の強制**では、すべての特権アクセスに対して MFA を必須とし、フィッシング攻撃や認証情報の漏洩に対する防御を強化します。FIDO2、WebAuthn、ハードウェアセキュリティキー（HSM）を活用した強力な認証を実装します。
...
**転送時暗号化（Encryption in Transit）**では、すべての通信チャネルで TLS 1.3以上を使用し、Perfect Forward Secrecy（PFS）を確保します。API ゲートウェイ、ロードバランサー、CDN などのサービスで、強力な暗号化設定を適用します。
