...
<a id="figure-multilayer-defense-architecture"></a>
![多層防御アーキテクチャの詳細設計](https://itdojp.github.io/it-infra-security-guide-book/assets/images/diagrams/chapter-chapter02/multilayer-defense-architecture.svg)

**外郭防御層**：ファイアウォール、IDS/IPS、WAF など、外部からの攻撃を最初に迎撃する層です。大量の攻撃トラフィックを識別・遮断し、明らかに悪意のあるアクセスを排除します。
...
**脅威インテリジェンスの共有**：外郭防御層で検知された攻撃パターンを内部防御層に伝達し、類似の攻撃に対する警戒レベルを高めます。例えば、ファイアウォールで特定の送信元 IP からの攻撃を検知した場合、内部システムでもその IP からのアクセスを重点的に監視します。
...
<a id="figure-implementation-priority-framework"></a>
![実装優先度フレームワーク](https://itdojp.github.io/it-infra-security-guide-book/assets/images/diagrams/chapter-chapter02/implementation-priority-framework.svg)
...
<a id="figure-security-architecture-three-elements"></a>
![セキュリティアーキテクチャの三要素統合](https://itdojp.github.io/it-infra-security-guide-book/assets/images/diagrams/chapter-chapter02/security-architecture-three-elements.svg)
...
**要件分析と設計判断**：ビジネス要件、コンプライアンス要件、技術的制約を総合的に分析し、最適なセキュリティアーキテクチャを設計します。例えば、新しい Web アプリケーションをデプロイする際に、パフォーマンス要件を満たしながら、OWASP Top 10の脅威に対する適切な保護を実装する設計を行います。
...
<a id="figure-infrastructure-engineer-roles-responsibilities"></a>
![インフラエンジニアのセキュリティ役割と責任範囲](https://itdojp.github.io/it-infra-security-guide-book/assets/images/diagrams/chapter-chapter02/infrastructure-engineer-roles-responsibilities.svg)
...
NIST Cybersecurity Framework 2.0（NIST CSF 2.0）は、「ガバナンス（Govern）」「識別（Identify）」「保護（Protect）」「検知（Detect）」「対応（Respond）」「復旧（Recover）」の6つの機能で、サイバーセキュリティリスクを組織横断で管理するためのフレームワークです。CSF 1.1 の5機能だけで成熟度を評価している場合は、2024年に公開された CSF 2.0 で追加された Govern 機能を明示的に取り込み、方針・責任分界・リスク受容・サプライチェーンリスクを技術対策の前提として扱います。

**ガバナンス（Govern）機能の実装**
セキュリティリスク管理の方針、役割と責任、リスク許容度、第三者・サプライチェーン管理、証跡要求を定義します。インフラエンジニアの視点では、クラウド責任共有モデル、管理者権限の棚卸し、例外承認、ログ保全、委託先や SaaS の責任分界を、設計レビュー時点で確認できる形にします。
...
**IaaS環境での適用**
ユーザーはゲスト OS 以上のセキュリティに責任を持ちます。OS 設定、アプリケーション設定、データ保護、ネットワーク設定などを適切に管理する必要があります。

**PaaS環境での適用**
ユーザーはアプリケーションコードとデータの保護に集中できますが、プラットフォーム設定の理解も重要です。API 設定、権限管理、ログ設定などに注意を払います。
...
<a id="figure-security-framework-implementation"></a>
![セキュリティフレームワークの実装アプローチ](https://itdojp.github.io/it-infra-security-guide-book/assets/images/diagrams/chapter-chapter02/security-framework-implementation.svg)
