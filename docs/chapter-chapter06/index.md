...
**転送時暗号化（Encryption in Transit）**では、すべての通信チャネルでTLSを有効化し、可能な範囲でTLS 1.3などの強度の高い設定とPerfect Forward Secrecy（PFS）を確保します。APIゲートウェイ、ロードバランサー、CDNなどのサービスで、強力な暗号化設定を適用します。
...
**クロスクラウドネットワーキング**では、VPN、専用線、SD-WANなどの技術を活用し、複数のクラウド間を安全に接続します。[第4章で学んだネットワークセグメンテーション](../chapter-chapter04/index.md#43-ネットワークセグメンテーション戦略)の概念を拡張し、クラウド間でのマイクロセグメンテーションを実現します。
