...
wrapperと親ディレクトリが管理対象ユーザーから書き込めないことを確認してから、次の内容を`/etc/sudoers.d/infra-operations`へ`root:root`、モード`0440`で配置します。sudoersのコマンド末尾の`""`は引数なしだけを許可する指定です。`PASSWD:`と利用者別の`timestamp_timeout=0`により、この例のコマンドでは毎回パスワード認証を要求します。
