...
> **この章で学ぶこと**
> - 情報セキュリティの本質と価値を理解する
> - CIA Triad（機密性・完全性・可用性）の実装方法を学ぶ  
...
![情報資産とセキュリティの全体像](../assets/images/diagrams/chapter-chapter01/information-assets-security-overview.svg)
...
このように、機密性は技術的手段だけでなく、物理的、手続き的な側面も含む包括的な概念です。インフラエンジニアが設計するシステムでは、これらすべての側面を考慮する必要があります。具体的な実装方法は、[第4章のネットワークセキュリティ](../chapter-chapter04/index.md#41-ファイアウォール設計と最適化)から[第7章のコンテナセキュリティ](../chapter-chapter07/index.md#71-コンテナセキュリティの基盤)で詳しく解説します。
...
![CIA Triad実装とトレードオフ分析](../assets/images/diagrams/chapter-chapter01/cia-triad-implementation.svg)
...
![インフラセキュリティ実装](../assets/images/diagrams/chapter-chapter01/infrastructure-security-implementation-mindmap.svg)
...
![脅威からビジネスインパクトまでのプロセスと防御ポイント](../assets/images/diagrams/chapter-chapter01/threat-to-business-impact-process.svg)
...
![リスクベースアプローチ実装手順](../assets/images/diagrams/chapter-chapter01/risk-based-approach-implementation.svg)

### 運用ルール例（最小）

基礎概念を実務に落とすには、判断を再現できるルールが必要です。初学者は、まず次を最小セットとして整備してください。

- 情報資産を分類し、取り扱い基準（保存先、共有方法、持ち出し）を定義する
- 受容可能リスクと例外の扱い（申請・承認、期限、代替策）を決める
- 重要操作（権限付与、ネットワーク開放、鍵・証明書更新など）は変更管理に載せ、監査ログを残す
- 可用性要件（RTO/RPO）をシステム単位で明文化し、復旧手順を定期的にテストする

> **よくある落とし穴**
> - ルールがなく、強い権限や例外が積み上がる
> - 冗長化しただけで安心し、復旧手順と運用訓練が不足する
...
**サプライチェーン攻撃**は、直接的な攻撃が困難な標的に対して、その供給業者やパートナー企業を経由して攻撃する手法です。信頼されたソフトウェアベンダーやサービスプロバイダーを攻撃することで、その顧客企業への侵入経路を確保します。前述のSolarWinds事件はこの典型例です。
...
![現代的サイバー攻撃の典型的フロー](../assets/images/diagrams/chapter-chapter01/modern-cyber-attack-sequence.svg)
