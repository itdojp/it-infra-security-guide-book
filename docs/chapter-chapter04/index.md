...
従来の「内部ネットワークは信頼できる」という前提に基づくVPN設計から、Zero Trust（ゼロトラスト）の原則に基づく「すべてを検証する」設計への移行が求められています。
...
これらのネットワーク監視・検知技術は、[第8章の継続的セキュリティ運用](../chapter-chapter08/index.md#82-siemsoar統合による高度な脅威検知)で扱う運用プロセスと統合することで、効果的なサイバーセキュリティ運用センター（SOC）を構築できます。
...
- VPN/リモートアクセスは原則としてMFAを必須化し、管理系は踏み台経由など経路を固定する
...
- Zero Trust（ゼロトラスト）原則に基づくVPN・リモートアクセスの再設計
